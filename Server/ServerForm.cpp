...
namespace Server
{
		   ...
		   (Int16::TryParse(textBox3->Text, period) == true))
		{
			if (period < 2880)
			{
				...
				if (nowTime->CompareTo(dateTimePicker1->Value) < 0)
				{
					...
					{
						MessageBox::Show("Некорректное расписание");
					}
				}
				else
				{
					MessageBox::Show("Невозможно сохранить устаревшее расписание");
				}
			}
			else
			{
				MessageBox::Show("Период слишком длинный");
			}
		}
	}
	...
	System::Void ClientForm::timer1_Tick(System::Object^  sender, System::EventArgs^  e)
	{
		...
		if (File::Exists(GetSchedule()->ip + ".txt") == false)
		{
